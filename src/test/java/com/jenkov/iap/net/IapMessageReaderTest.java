...
public class IapMessageReaderTest {
    ...
    public void testReadIapMessage() {
        ...
        int bytesWritten = IapMessageWriter.writeIapMessageHeaders(dest, 0, sourceMessage);
        assertEquals(45, bytesWritten);

        int index = 0;
        bytesWritten = IapMessageWriter.writeIapMessageHeaders(dest, index, sourceMessage);


        IapMessageReader iapMessageReader = new IapMessageReader();

        iapMessageReader.setSource(dest, index, bytesWritten);
        ...
        assertEquals(3, destMessage.senderIdOffset);
        assertEquals(3, destMessage.senderIdLength);

        assertEquals(9, destMessage.receiverIdOffset);
        ...
        assertEquals(30, destMessage.semanticProtocolIdOffset);
        assertEquals(3, destMessage.semanticProtocolIdLength);

        assertEquals(36, destMessage.semanticProtocolVersionOffset);
        assertEquals(3, destMessage.semanticProtocolVersionLength);

        assertEquals(42, destMessage.messageTypeOffset);
